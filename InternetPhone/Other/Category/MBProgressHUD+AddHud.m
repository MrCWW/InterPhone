...
+ (MBProgressHUD *)lc_showMessag:(NSString *)message toView:(UIView *)view{
...
#pragma mark -- /**有文字带小菊花带遮罩view不会自动消失*/
+ (MBProgressHUD *)showMessag:(NSString *)message toView:(UIView *)view dimBackground:(BOOL)isDim {
    if (view == nil) view = [UIApplication sharedApplication].keyWindow;
    // 快速显示一个提示信息
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:view animated:YES];
    hud.labelText = message;
...
#pragma mark -- /**有文字带小菊花带遮盖,NSTimeInterval传0默认是2秒*/
+ (void)showMessag:(NSString *)message toView:(UIView *)view afterDelay:(NSTimeInterval)timeInterval dimBackground:(BOOL)isDim {
    MBProgressHUD * hud = [self showMessag:message toView:view];
    hud.dimBackground = isDim;
    [hud hide:YES afterDelay:timeInterval?timeInterval:2];
}
